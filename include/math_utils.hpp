                                                ...
                                                const T &gamma)
{
  ...
  */
  // Handle precision cases
  T gamma_cos = (std::abs(std::cos(gamma)) < 5e-7) ? 0 : std::cos(gamma);
  T beta_cos = (std::abs(std::cos(beta)) < 5e-7) ? 0 : std::cos(beta);
  T alpha_cos = (std::abs(std::cos(alpha)) < 5e-7) ? 0 : std::cos(alpha);

  T omega = (alpha_cos - gamma_cos * beta_cos) / (std::sin(gamma));

  _transform_matrix[0] = a;
  _transform_matrix[1] = T(0);
  _transform_matrix[2] = T(0);

  _transform_matrix[3] = b * gamma_cos;
  _transform_matrix[4] = b * std::sin(gamma);
  _transform_matrix[5] = T(0);

  _transform_matrix[6] = c * beta_cos;
