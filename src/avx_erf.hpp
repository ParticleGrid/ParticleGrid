...
inline v8sf pointwise_exp_avx(v8sf x){
    for(int i = 0; i < 8; i++){
        x[i] = exp(x[i]);
    }
    return x;
...
inline float erf_apx(float x){
    x = exp(-x*x);
    return float(1/SQRT_PIO2)*sqrt(1-x)*(float(SQRT_PIO2) + (float(31)/200)*x - (float(341)/8000)*x*x);
}
